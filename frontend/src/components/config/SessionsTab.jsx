        ...
        <div className="space-y-2">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              onChange={(e) => updateConfigValue('SERVER', 'PICKUP_MODE_ENABLED', e.target.checked ? 1 : 0)}
            />
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-4">
          <label className={`label whitespace-nowrap min-w-[5rem] text-sm ${!isEnabled ? 'opacity-50' : ''}`}>
            Qualify limit: <span className="font-semibold text-blue-600 dark:text-blue-400">{config?.QUALIFY?.QUALIFY_MAX_WAIT_PERC || 120}%</span>
          </label>
          <div className="flex-1 relative" style={{ top: '-7px' }}>
            <input
              type="range"
  ...
  return (
    <div className="card">
      ...
      <div className="space-y-4">
        {/* Limit by dropdown */}
        <div className="flex items-center gap-4">
          <label className={`label whitespace-nowrap min-w-[5rem] text-sm ${!isEnabled ? 'opacity-50' : ''}`}>Limit by:</label>
          <select
        ...
        {config?.RACE?.LAPS !== undefined ? (
          <div className="flex items-center gap-4">
            <label className={`label whitespace-nowrap min-w-[5rem] ${!isEnabled ? 'opacity-50' : ''}`}>
              Laps: <span className="font-semibold text-blue-600 dark:text-blue-400">{config?.RACE?.LAPS || 5}</span>
            </label>
            <div className="flex-1 relative" style={{ top: '-7px' }}>
              <input
                type="range"
          ...
          <div className="flex items-center gap-4">
            <label className={`label whitespace-nowrap min-w-[5rem] ${!isEnabled ? 'opacity-50' : ''}`}>
              Time: <span className="font-semibold text-blue-600 dark:text-blue-400">
                {String(Math.floor((config?.RACE?.TIME || 0) / 60)).padStart(2, '0')}:{String((config?.RACE?.TIME || 0) % 60).padStart(2, '0')}
              </span>
            </label>
            <div className="flex-1 relative" style={{ top: '-7px' }}>
              <input
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 relative" style={{ top: '-7px' }}>
            <input
              ...
              onChange={(e) => updateConfigValue('SERVER', 'RACE_OVER_TIME', parseInt(e.target.value))}
              disabled={!isEnabled}
            />
          </div>
        </div>
        ...
        {/* Join Type Dropdown */}
        <div className="flex items-center gap-4">
          <label className={`label whitespace-nowrap min-w-[5rem] text-sm ${!isEnabled ? 'opacity-50' : ''}`}>Join type:</label>
          <select
            className="flex-1 px-3 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded text-gray-900 dark:text-gray-100"
            ...
            <div className={`grid grid-cols-2 gap-4 ml-6 ${!isEnabled ? 'opacity-50' : ''}`}>
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) => updateConfigValue('RACE', 'MANDATORY_PIT_FROM', parseInt(e.target.value))}
                  disabled={!isEnabled}
                />
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) => updateConfigValue('RACE', 'MANDATORY_PIT_TO', parseInt(e.target.value))}
                  disabled={!isEnabled}
                />
                <span className="text-gray-600 dark:text-gray-400 text-sm">lap</span>
              </div>
